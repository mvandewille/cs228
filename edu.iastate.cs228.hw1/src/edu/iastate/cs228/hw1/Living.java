...
public abstract class Living 
{
	...
	protected void census(int population[])
	{
		...
		int[] popArray = population;
		int w = plain.getWidth();
		int testRow;
		int testColumn;
		//runs through a 3x3 grid centered at the current Living type's location
		for (int j = -1; j < 2; j++)
		{
			testRow = row + j;
			for (int k = -1; k < 2; k++)
			{
				testColumn = column + k;
				//if the location within the 3x3 grid is inside the larger grid, then check for which Living type and add to population array
				if (testRow >= 0 && testRow < w && testColumn >= 0 && testColumn < w)
				{
					if (plain.grid[testRow][testColumn].who() == State.BADGER)
					{
						popArray[BADGER]++;
					}
					if (plain.grid[testRow][testColumn].who() == State.EMPTY)
					{
						popArray[EMPTY]++;
					}
					if (plain.grid[testRow][testColumn].who() == State.FOX)
					{
						popArray[FOX]++;
					}
					if (plain.grid[testRow][testColumn].who() == State.GRASS)
					{
						popArray[GRASS]++;
					}
					if (plain.grid[testRow][testColumn].who() == State.RABBIT)
					{
						popArray[RABBIT]++;
					}
				}
				//if the location in the 3x3 grid is outside the larger grid, ignore it and move to the next location
			}
		}
	}
