...
public class RotationalPointScanner  
{
	...
	protected RotationalPointScanner(String inputFileName, Algorithm algo) throws FileNotFoundException, InputMismatchException
	{
		...
		Scanner countScan = new Scanner(input);
		...
		for (int i = 0; i < (coordCount/2); i++)
		{
			for (int j = 0; j < 2; j++)
			{
				if (j == 0)
				{
					xCoord = fileScan.nextInt();
				}
				else
				{
					yCoord = fileScan.nextInt();
				}
			}
	...
	public void scan()
	{
		...
		aSorter.sort();
		scanTime = System.nanoTime() - compareTime;
	}
	...
	{
		String statReturn = "";
		statReturn = sortingAlgorithm.name() + "\t" + points.length + "\t" + scanTime;
		return statReturn;
	}
	...
	public String toString()
	{
		...
		for (int i = 0; i < points.length; i++)
		{
			returnString = returnString + points[i].getX() + " " + points[i].getY() + "\n";
		}
		return returnString;
	}
	...
	public void writePointsToFile() throws FileNotFoundException
	{
		...
		PrintWriter p = new PrintWriter(printFile);
		p.print(toString());
		p.close();
	}	
	...
	{		
		...
		{
			segments[points.length + j] = new Segment(points[j], medianCoordinatePoint);
		}
